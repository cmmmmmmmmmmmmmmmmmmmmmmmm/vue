<template>
  <div class="dispose">
    ...
    <el-card v-for="item in list" :key="item" class="base-info-item">
      ...
      <div class="base-info-item">
        <span> 申诉结果：</span>{{ item.messageStatus }}
      </div>

      <div class="base-info-item" v-show="item.messageStatus === '驳回'">
        <span> 驳回原因：</span>{{ item.messageReason }}
      </div>

      <template #footer>
        <div class="footer" v-show="item.messageStatus === '驳回'">
          <el-button type="primary" @click="onDispose(item)">
            再次申诉
          </el-button>
        </div>
      </template>
  ...
  methods: {
    ...
    getList() {
      ...
      axios.get('/js/appeal.json').then(
        (res) => {
          let currentDate = res.data.data.list.slice(0, 6)
          let start = (data.pageNumber - 1) * data.pageSize
          let end = data.pageNumber * data.pageSize

          this.list = currentDate.slice(start, end)
          this.pagiParams.total = 6
        },
        (res) => {
          console.log('error')
        }
